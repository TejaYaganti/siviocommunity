...
function Extracurricular() {
    ...
    useEffect(() => {
        ...
        axios.post(`${pythonDomain}extraDetails`, { email: token })
            .then(response => {
    ...
    const onSave = () => {
        ...
        axios.post(`${pythonDomain}newExtraRecord?extraRecord=${JSON.stringify(extraRecordData)}`, { email: token })
            .then(response => {
    ...
    return (
        <div>
            <div className='PageTitle'>Extracurricular Activities</div>
            {loading && (
            ...
            <div className='FamilyTable'>
                <table style={{ width: '800px', paddingTop: '10px' }}>
                    <thead>
                        <tr>
                            <th style={{ width: '25%', textAlign: 'left' }}>Order of Importance</th>
                            <th style={{ width: '25%', textAlign: 'left' }}>Type</th>
                            <th style={{ width: '25%', textAlign: 'left' }}>Organization Name</th>
                    ...
                    <tbody>
                        {extraList.map((ext) => (
                            <tr key={ext.Id}>
                                <td>{ext.Order_of_Importance__c}</td>
                                <td>{ext.Type__c}</td>
                                <td>{ext.Organization_Name__c}</td>
                                <td>{ext.Role__c}</td>
                                <td>
